...
ap.add_argument('--batch_size', type=int,
                help='Number of 3D voxel batches', default=16)
ap.add_argument('--lr', type=float,
                help='Initial Learning rate', default=0.001)
ap.add_argument('--lr_decay', type=float,
                help='Learning Rate Decay', default=0.1)
ap.add_argument('--optimizer', help='sgd, adam', default='adam')
ap.add_argument(
    '--epochs', help='Total number of epochs to train on data', default=200, type=int)
ap.add_argument(
    '--iters', help='Number of training batches per epoch', default=None, type=int)
ap.add_argument('--aug', action='store_true',
                help='Flag to decide about input augmentations', default=False)
...
def get_model(mode, flag_3d=True, channel_size_3d=32, mri_slice_dim=128):
    ...
    # Image dims must be powers of 2
    assert math.log(mri_slice_dim, 2).is_integer()
    ...
    t1_lgg = MRIDataset(root=root,  mode=mode, channel_size_3d=channel_size_3d,
                        flag_3d=flag_3d, mri_slice_dim=mri_slice_dim, aug=aug)
    dataset = t1_lgg
...
if not args.demo:
    ...
    primary_data_loader = DataLoader(
        primary_dataset, args.batch_size, shuffle=True, num_workers=0, collate_fn=mt_datasets.mt_collate)
    val_data_loader = DataLoader(val_dataset, args.batch_size,
                                 shuffle=True, num_workers=0, collate_fn=mt_datasets.mt_collate)
    add_to_log("training on %d samples" % len(primary_dataset))
else:
    primary_dataset = get_model('test')
    primary_data_loader = DataLoader(
        primary_dataset, shuffle=False, batch_size=1)
...
def train(epoch, losstype='dice'):
    ...
    if args.iters is not None:
        primary_data_loader.dataset.segmentation_pairs = primary_data_loader.dataset.segmentation_pairs[
            :args.iters]
    ...
    for idx, (inp, seg) in enumerate(primary_data_loader):
        ...
        optimizer.zero_grad()
        inp, seg = torch.tensor(inp).cuda(3), torch.tensor(
            seg, requires_grad=False).cuda(3)
        out = net.forward(inp)
        ...
        log_str = add_to_log("Epoch %d, Batch %d/%d: Loss=%0.6f" %
                             (epoch, idx+1, len(primary_data_loader), avg_tool.get_average()))
        loss.backward()
        optimizer.step()
...
def validate(losstype):  # num_patients=20):
    ...
    if args.iters is not None:
        val_data_loader.dataset.segmentation_pairs = val_data_loader.dataset.segmentation_pairs[
            :args.iters]
...
if not args.demo:
    ...
    if args.load_from is not None:
        if os.path.isfile(args.load_from):
            log_str = add_to_log(
                "=> loading checkpoint '{}'".format(args.load_from))
            ...
            log_str = add_to_log(
                "=> no checkpoint found at '{}'".format(args.load_from))
    else:
        start = 0
